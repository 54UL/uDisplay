...
void st7735_init(uDisplayUnderlyingProtocol *protocol) 
{
    ...
    // Below is a basic initialization sequence for ST7735
    uint8_t exit_sleep_mode[] = { 0x11 };
    st7735_send_command(exit_sleep_mode, sizeof(exit_sleep_mode));
    ...
    st7735_send_command(pixel_format_set, sizeof(pixel_format_set));

    // Memory Data Access Control
    uint8_t mem_data_access_control[] = { 0x36, 0x00 };
    st7735_send_command(mem_data_access_control, sizeof(mem_data_access_control));

    // Inversion Control
    uint8_t inversion_control[] = { 0xB4, 0x00 };
    st7735_send_command(inversion_control, sizeof(inversion_control));

    // Porch control
    uint8_t porch_control[] = { 0xB2, 0x0C, 0x0C, 0x00, 0x33, 0x33 };
    st7735_send_command(porch_control, sizeof(porch_control));

    // Gamma Set
    uint8_t gamma_set[] = { 0x26, 0x01 };
    st7735_send_command(gamma_set, sizeof(gamma_set));

    // Display On
    uint8_t display_on[] = { 0x29 };
    st7735_send_command(display_on, sizeof(display_on));
