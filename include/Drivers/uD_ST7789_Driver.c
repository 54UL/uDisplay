...
void st7789_init(uDisplayUnderlyingProtocol *protocol) 
{
    ...
    st7789_driver.SendCommand(exit_sleep_mode, sizeof(exit_sleep_mode));
    ...
    st7789_driver.SendCommand(pixel_format_set, sizeof(pixel_format_set));

    // Memory Data Access Control
    uint8_t mem_data_access_control[] = { 0x36, 0x00 };
    st7789_driver.SendCommand(mem_data_access_control, sizeof(mem_data_access_control));

    // Inversion Control
    uint8_t inversion_control[] = { 0xB4, 0x00 };
    st7789_driver.SendCommand(inversion_control, sizeof(inversion_control));

    // Porch control
    uint8_t porch_control[] = { 0xB2, 0x0C, 0x0C, 0x00, 0x33, 0x33 };
    st7789_driver.SendCommand(porch_control, sizeof(porch_control));

    // Gamma Set
    uint8_t gamma_set[] = { 0x26, 0x01 };
    st7789_driver.SendCommand(gamma_set, sizeof(gamma_set));

    // Display On
    uint8_t display_on[] = { 0x29 };
    st7789_driver.SendCommand(display_on, sizeof(display_on));
